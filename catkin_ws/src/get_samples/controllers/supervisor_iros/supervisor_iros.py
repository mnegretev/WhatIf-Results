...
def main():
    ...
    print('STARTING SUPERVISOR FOR WHAT-IF TESTS... September 2024')
    ...
    cars = [robot.getFromDef('vehicle_1'), robot.getFromDef('vehicle_2'),
            robot.getFromDef('vehicle_3'), robot.getFromDef('vehicle_4')]
    tf = []
    i = 0
    for car in cars:
        if car is not None:
    ...
    while robot.step(TIME_STEP) != -1 and not rospy.is_shutdown():
        if lateral_car_1 is not None: lateral_car_1.setVelocity([0, speed_cars_right_lane, 0, 0, 0, 0])
        if lateral_car_2 is not None: lateral_car_2.setVelocity([0, speed_cars_left_lane, 0, 0, 0, 0])
        i = 0
        # print(cars)
        for car in cars:
            if car is not None:
