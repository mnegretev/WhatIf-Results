...
def main():
    ...
    print('STARTING SUPERVISOR FOR WHAT-IF TESTS... Augusts 2024')
    ...
    while robot.step(TIME_STEP) != -1 and not rospy.is_shutdown():
        if lateral_car_1 is not None: lateral_car_1.setVelocity([0, -5, 0, 0, 0, 0])
        if lateral_car_2 is not None: lateral_car_2.setVelocity([0, -6, 0, 0, 0, 0])
        i = 0
        # print(cars)
        for car in cars:
            if car is not None:
