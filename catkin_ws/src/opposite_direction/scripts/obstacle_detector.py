...
def callback_point_cloud(msg):
    ...
    N_points  = xyz[(xyz[:,0] >  2.5) & (xyz[:,0] <  25) & (xyz[:,1] < 2.55) & (xyz[:,1] > -2.55)]
    NW_points = xyz[(xyz[:,0] >  5) & (xyz[:,0] <   25) & (xyz[:,1] < 9.5) & (xyz[:,1] >  3.5)]
    W_points  = xyz[(xyz[:,0] > -5) & (xyz[:,0] <  5) & (xyz[:,1] < 9.5) & (xyz[:,1] >  3.5)]
    SW_points = xyz[(xyz[:,0] >  -25) & (xyz[:,0] < -5) & (xyz[:,1] < 9.5) & (xyz[:,1] >  3.5)]
    
    NE_points  = xyz[(xyz[:,0] >  5) & (xyz[:,0] <   25) & (xyz[:,1] > -9.5) & (xyz[:,1] <  -3.5)]        
    E_points  = xyz[(xyz[:,0] > -5) & (xyz[:,0] <  5) & (xyz[:,1] > -9.5) & (xyz[:,1] <  -3.5)] 
    SE_points  = xyz[(xyz[:,0] >  -25) & (xyz[:,0] < -5) & (xyz[:,1] > -9.5) & (xyz[:,1] <  -3.5)]               
    ...
    obs_points  = xyz[(xyz[:,0] >  2.5) & (xyz[:,0] <  50) & (xyz[:,1] < 2.55) & (xyz[:,1] > -2.55)]  
...
def main():
    ...
    print("INITIALIZING OBSTACLE DETECTOR (NOV 2024)...", flush=True)
    ...
    pub_obs_dist = rospy.Publisher("/obstacle/dist_north", Float64, queue_size=1)
