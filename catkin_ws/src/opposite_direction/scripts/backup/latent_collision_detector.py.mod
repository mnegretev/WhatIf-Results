...
def main():
    global latent_collision, curr_time, sdc_vel_kh, sdc_steer, curr_lane, free_N, free_NW, free_W, free_NE, free_E, action, radar_scan, free_left, free_right    
    ...
    rospy.Subscriber("/radar_scan", RadarScan, callback_radar)
    ...
    while not rospy.is_shutdown():
       ...
       if closest_obst_front != None:

          obst_at_front = True 
          #print("Azimuth=", closest_obst_front.azimuth, flush = True) 
         
          # Ensure the detected distance stays within bounds
          closest_obst_front_range = max(radar_ref["min_dist"],
                                         min(closest_obst_front.range,
                                         radar_ref["max_dist"])
                                        )
