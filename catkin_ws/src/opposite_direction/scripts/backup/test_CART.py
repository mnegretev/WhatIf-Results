...
def main(speed_left, speed_right):
    ...
    path = os.getcwd()
    file = path + "/src/opposite_direction/scripts/decision_models/CART_10.cart"
    print(file, flush=True)    
    try:
       model = pickle.load(open(file, 'rb'))  
    except Exception as error:
       print("Error loading model:", error)
       sys.exit("Please enter to the subdirectory ./decision_models and run 'sh script.sh' on terminal")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
       print("Usage: rosrun opposite_direction  test_CART.py speed_left speed_right (m/s)\nSuggested values speed_left=5 speed_right=3", len(sys.argv))
    else:   
       try:
