...
def main():
    ...
    global enable_cruise, enable_follow, dist_north, start_change_lane_on_left, start_change_lane_on_right
    ...
    rospy.Subscriber("/start_swerve_left", Bool, callback_start_swerve_left)
    rospy.Subscriber("/start_swerve_right", Bool, callback_start_swerve_right)
    rospy.Subscriber("/obstacle/dist_north", Float64, callback_dist_north)
    rospy.Subscriber("/self_driving_pose", Pose2D, callback_current_pose)
    ...
    while not rospy.is_shutdown():
        if state == SM_INIT:
            ...
            speed,steering = calculate_control(lane_rho_l,lane_theta_l,lane_rho_r,lane_theta_r,goal_rho_l,goal_theta_l,goal_rho_r,goal_theta_r,dist_north)
            #print([goal_rho_l,goal_theta_l,goal_rho_r,goal_theta_r], flush=True)
        ...
        #
            ...
            #print("state=", state, "abort=", abort, sep = " ", flush = True)
            if speed <=10:
                speed = max_speed
            steering = calculate_turning_steering(-1.2, 2.9, speed)
        ...
        #
          ...
          if current_y < y_thresh:
             if speed <= 10:
                speed = max_speed
             steering = calculate_turning_steering(1.2, 2.9, speed)
             print("start_swerve_left", flush = True )
          else:
            ...
            steering = calculate_turning_steering(-1.2, 2.9, speed)
            if current_y > y_thresh or abs(current_a) < 0.01:
        ...
        if state in undoable_states and abort:
