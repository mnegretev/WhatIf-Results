...
def publish_action(action, prev_action, abort, obs_state, prev_obs_state, model, latent_collision, prev_latent_collision):
    ...
    if (not all(obs_state[key] == prev_obs_state[key] for key in obs_state)) or prev_latent_collision != latent_collision or prev_action != action:
       ...
       if action != "cruise" and action != "keep" and action != "swerve_left" and action != "swerve_right":
          #if not abort:             
...
def main(speed_left, speed_right, MLP_path, counterfactuals_path):
    ...
    # Dictionary of the observed state of the environment
    obs_state = {}
    ...
    # Delay the selection of actions for letting the environment data to arrive
    ...
    while not rospy.is_shutdown():
      ...
      #print("DFA STATE=", dfa_state, flush = True)
      #print("obs_state=", obs_state, flush = True)
      #print("prev_obs_state=", prev_obs_state, flush = True)      
      ...
      #print("dfa_state=",dfa_state, "prev_latent_collision=", prev_latent_collision, "latent_collision=", latent_collision, flush=True)
      #print("obs_state=", obs_state, flush=True)
      if (not all(obs_state[key] == prev_obs_state[key] for key in obs_state)) or not (prev_latent_collision and latent_collision):
         if dfa_state == DFA_INIT:
            # input
            if latent_collision:
               # output
               abort = False
               ...
               # Transition
               dfa_state = DFA_WHATIF            
         elif dfa_state == DFA_PERCEPTRON:
            # input
            if latent_collision:
               # output
               abort = False
         ...
         prev_obs_state = obs_state.copy()
         # Store previous dfa_state of latent collision
