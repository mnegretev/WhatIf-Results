...
def main():
    global latent_collision, curr_time, sdc_vel_kh, curr_lane, free_N, free_NW, free_W, free_NE, free_E,  free_SE, free_SW, action, radar_scan 
    ...
    transversal = False
    if rospy.has_param('~transversal'):
        transversal = rospy.get_param('~transversal')
    ...
    pub_type_latent_collision  = rospy.Publisher("/type_latent_collision", Float64, queue_size=1)        
    ...
    while not rospy.is_shutdown():
       ...
       if action == "swerve_left" or action == "swerve_right" or transversal:
          radar_ref["fov"] = 0.8725 # 50 degrees each side
       
       # Get the closest vehicle in front 
       for radar in radar_scan.returns:
         if (-radar_ref["fov"] <= radar.azimuth <= radar_ref["fov"] 
               and radar.range <= valid_dist
            ):
       ...
       if action == 'change_to_right' and curr_lane:
       ...
       # sideSwipe collision   
          ...
          print("Latent collision while changing to left", free_NW, free_W, free_SW, sep = " ", flush = True)
       # sideSwipe collision
       elif action == 'change_to_right' and not curr_lane and (not free_NE or not free_E):   
          latent_collision = True
          type_latent_collision = 4.0
          print("Latent collision while changing to right", free_NE, free_E, free_SE, sep = " ", flush = True)
       # free to go    
       elif action == 'change_to_left' and curr_lane and free_NW and free_W and free_SW:
          latent_collision = False
          type_latent_collision = -1.0
       # free to go    
       elif action == 'change_to_right' and not curr_lane and free_NE and free_E and free_SE:
          latent_collision = False
          type_latent_collision = -1.0
       ...
       pub_type_latent_collision.publish(type_latent_collision)
